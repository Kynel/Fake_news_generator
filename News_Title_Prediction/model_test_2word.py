...
sources, targets = [], []
with open(os.path.join(ROOT_DIR,'Data_Preprocess','Data','sources2.dat'), 'rb') as sources_stream:
    sources = pickle.load(sources_stream)
with open(os.path.join(ROOT_DIR,'Data_Preprocess','Data','targets2.dat'), 'rb') as targets_stream:
    targets = pickle.load(targets_stream)

source2idx, idx2source, target2idx, idx2target = None, None, None, None

with open(os.path.join(os.path.dirname(__file__), 'data_out', 'source2idx_2.dat'), 'rb') as source2idx_stream:
    source2idx = pickle.load(source2idx_stream)
with open(os.path.join(os.path.dirname(__file__), 'data_out', 'idx2source_2.dat'), 'rb') as idx2source_stream:
    idx2source = pickle.load(idx2source_stream)
with open(os.path.join(os.path.dirname(__file__), 'data_out', 'target2idx_2.dat'), 'rb') as target2idx_stream:
    target2idx = pickle.load(target2idx_stream)
with open(os.path.join(os.path.dirname(__file__), 'data_out', 'idx2target_2.dat'), 'rb') as idx2target_stream:
...
manager = tf.train.CheckpointManager(ckpt, './data_out/training_checkpoints/2words_input', max_to_keep=3)
...
def run():
    ...
    if(args == '1'):
        ...
        while(True):
            sentence = input("띄어쓰기로 구분된 두 단어 입력.\n")
            try:
